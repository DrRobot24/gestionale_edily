...
| [`note-contabili-contabilizzata.sql`](supabase/schema/note-contabili-contabilizzata.sql) | ⏳ **DA ESEGUIRE** (scritto il 2026-09-25). Aggiunge a `note_contabili` `contabilizzata_il` (timestamptz, NULL = non ancora) e `contabilizzata_da`: il flag «contabilizzato» dei Lavori extra. Nessuna policy nuova, scrive chi può già correggere la nota (tecnico sui suoi cantieri, titolare ovunque). Finché non gira, la pagina funziona senza il flag: la lettura ripiega da sola se la colonna manca (42703) |
