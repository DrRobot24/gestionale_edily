...
| [`scheda-patenti-dpi.sql`](supabase/schema/scheda-patenti-dpi.sql) | ⏳ **DA ESEGUIRE PRIMA DEL PUSH** (scritto il 2026-09-25): il frontend legge `patenti` e `dpi_consegnati`, senza le colonne Risorse e squadre vanno in errore. Due jsonb sulla riga della persona: patenti `[{tipo, conseguita_il, scade_il}]` (spunta + righe, almeno una data) e DPI `[{dpi, consegnato_il}]` (ogni spunta chiede il giorno). Restano nella scheda anche da archiviata. Ricopia `patente` (spezzato sulle virgole) e `dpi` senza date, che il modulo chiederà al primo salvataggio; le colonne vecchie restano e il modulo le tiene allineate |
