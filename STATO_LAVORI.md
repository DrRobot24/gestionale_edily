...
| [`risorse-impiego-stipendio.sql`](supabase/schema/risorse-impiego-stipendio.sql) | ✅ eseguito il 2026-09-24, verificato: 3 policy, 2 trigger, 1 check, nessun assunto senza impiego. Aggiunge la data di impiego (un trigger la copia dall'assunzione se manca, un check vieta l'impiego dopo l'assunzione) e la tabella `dipendente_stipendi`, storico per data, letta solo con `paghe.read` perché `dipendenti` la legge anche il tecnico. **Dal 2026-09-24 in pagina si chiama «Salario»**: il fisso pattuito a voce, da cui Stefania ricava la tariffa oraria. **Regola della tariffa decisa il 2026-09-25** (vedi `retribuzione.ts`): paga mensile OPPURE tariffa oraria, mai insieme — vale la riga più recente fra `dipendente_stipendi` e `dipendente_costi`. Con la paga globale la tariffa del mese = paga ÷ (giorni lavorabili del mese, lun–ven sul calendario × ore della giornata piena da contratto) — **corretto dall'utente la sera del 25**, la prima versione divideva per le ore validate e dava 44,44 €/h a metà mese. Il costo del mese è sempre la paga intera; straordinario alla stessa tariffa; trasferta esclusa. Non si scrive nel database: si ricalcola da paga e ore. In scheda un riquadro solo, «Retribuzione». Tolto dal modulo delle tariffe il prezzo di vendita |
...
| [`orario-contrattuale.sql`](supabase/schema/orario-contrattuale.sql) | ✅ eseguito il 2026-09-25, verificato: `app.ore_contratto` risponde 8 per tutte e dieci le persone (nessun part-time ancora scritto). Il part-time: tabella `dipendente_orari` (storico per data, ore al giorno; senza righe vale 8; scrive `anagrafiche.write`, legge `anagrafiche.read`) e funzione `app.ore_contratto(dipendente, giorno)`. **Ridefinisce due funzioni**, copiate dai loro file che sono stati aggiornati allo stesso modo: `ore_griglia` (l'assenza a giornata intera vale le ore del contratto, non 8) e `invia_foglio_giornata` (il controllo delle ore misura ognuno col suo contratto; e «nessuno dimenticato» ora guarda la **messa in servizio**, non l'assunzione — difetto nato il 25 con il principio del servizio). Non provato su un Postgres di prova. In fondo elenca le ore di oggi per persona: senza orari scritti devono essere tutte 8 |
| [`riepilogo-economico.sql`](supabase/schema/riepilogo-economico.sql) | ✅ eseguito il 2026-09-25 (conferma dell'utente, risultato della verifica non visto). Il Riepilogo economico del mese: `paghe_mesi` (stato bozza → inviato → validato), `paghe_movimenti` (acconti, rimborsi, trattenute, sempre col motivo; scrivibili solo a mese in bozza), `paghe_righe` (la fotografia scattata all'invio, che il titolare firma e che resta in archivio). Tre funzioni `security definer`: `invia_paghe` (paghe.read), `decidi_paghe` e `riapri_paghe` (rapportini.validate). Validando, i rapportini validati del mese passano a `contabilizzato` (azzurro, archiviata); riaprendo tornano `validato`. **Non provato su un Postgres di prova** (Docker spento il 25): alla prima esecuzione controllare la verifica in fondo, e al primo giro vero la transizione dei rapportini. Senza il file la pagina mostra un avviso e la home del titolare non cambia |
...
> **Aggiornamento del 2026-09-25 — il netto non si scrive più a mano.** È nato il
> **Riepilogo economico** del mese (`/riepilogo-economico`, vedi
> `riepilogo-economico.sql`): calcola per ogni persona quanto ha maturato (paga
> globale o giornaliera, `retribuzione.ts`), con acconti, rimborsi e trattenute, e
> il titolare lo firma. La colonna «Netto in busta» di questa pagina è diventata
> **«Percepito»** e legge dal riepilogo firmato (maturato + rimborsi − trattenute);
> `buste_paga` e il netto dal cedolino **non servono più** — nel piano qui sotto
> il punto 1 si riduce a `monte_ferie`, e se si esegue `economia-risorse.sql` la
> tabella `buste_paga` resta inutilizzata.
